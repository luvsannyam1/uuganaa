...
//   ];
  const BusPath = new google.maps.Polyline({
    path: dummyData[2].shape.vertices,
    geodesic: true,
    strokeColor: dummyData[2].color,
    strokeOpacity: 1.0,
    strokeWeight: 2,
  });
      ...
      <div style={{zIndex: '10', position: 'absolute' , right: '10px', backgroundColor: 'white', padding: '40px', borderRadius: '10px', top: '40%', flexDirection:'column'}} className=''>
        {/* <Product /> */}
       {
         Data.map((item, idx) => {
           return(
             <>
            <h1 onClick={()=>{handleBusRoute(idx)}} key={idx} style={{cursor:'pointer'}}>{item.name}</h1>
            </>
           )
         })
       }
