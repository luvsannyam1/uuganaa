...
import { useJsApiLoader, GoogleMap, Autocomplete, DirectionsRenderer, Polyline,} from '@react-google-maps/api'
 ...
 BusPath.setMap(map);
  ...
  return (
    ...
    >
      ...
      <div style={{zIndex: '10', position: 'absolute' , right: '10px', backgroundColor: 'white', padding: '40px', borderRadius: '10px', top: '40%', flexDirection:'column'}} className=''>
        {/* <Product /> */}
       {
         Data.map((item, idx) => {
           return(
             <>
            <h1 onClick={(e) => handleBusRoute(idx)} style={{cursor:'pointer' , zIndex: '100'}}>{item.name}</h1>
            </>
           )
         })
       }
